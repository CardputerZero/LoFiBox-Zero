...
"""Navidrome provider plugin module."""
...
def dispatch(method: str, params: Dict[str, Any]):
    ...
    if method == "probe":
        return navidrome_provider.probe(p["profile"])
    if method == "search":
        return tracks_response(p["profile"], navidrome_provider.search(
            p["profile"], p["query"], int(p.get("limit", 20))))
    if method == "recent":
        return tracks_response(p["profile"], navidrome_provider.recent(
            p["profile"], int(p.get("limit", 20))))
    if method == "library_tracks":
        return tracks_response(p["profile"], navidrome_provider.library_tracks(
            p["profile"], int(p.get("limit", 50))))
    if method == "browse":
        return nodes_response(p["profile"], navidrome_provider.browse(
            p["profile"], p.get("parent", {}), int(p.get("limit", 50))))
    if method == "resolve":
        return stream_response(p["profile"], navidrome_provider.resolve(
            p["profile"], p["track"]))
    return None
