...
"""Jellyfin provider plugin module."""
...
# --- Per-method field contracts ---
# Keys: method name
# Values: (required_fields, known_fields)
# known_fields includes both required and optional.  Unknown keys in params
# are logged to stderr so mapping drift is caught in development.
...
def _validate_params(method: str, params: Dict[str, Any]) -> None:
    """Check required fields present; warn on unknown keys."""
    schema = _METHOD_SCHEMAS.get(method)
    if schema is None:
        return  # unknown method: dispatch returns None later
...
def dispatch(method: str, params: Dict[str, Any]):
    ...
    if method == "probe":
        return jellyfin_provider.probe(p["profile"])
    if method == "search":
        return tracks_response(p["profile"], jellyfin_provider.search(
            p["profile"], p.get("session", {}), p["query"], int(p.get("limit", 20))))
    if method == "recent":
        return tracks_response(p["profile"], jellyfin_provider.recent(
            p["profile"], p.get("session", {}), int(p.get("limit", 20))))
    if method == "library_tracks":
        return tracks_response(p["profile"], jellyfin_provider.library_tracks(
            p["profile"], p.get("session", {}), int(p.get("limit", 50))))
    if method == "browse":
        return nodes_response(p["profile"], jellyfin_provider.browse(
            p["profile"], p.get("session", {}), p.get("parent", {}), int(p.get("limit", 50))))
    if method == "resolve":
        return stream_response(p["profile"], jellyfin_provider.resolve(
